...
class EthiopianEnglishBot:
    ...
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        for lang_key, lang_data in self.vocabularies.items():
            ...
            keyboard.append(
                [InlineKeyboardButton(button_text, callback_data=f"lang_{lang_key}")]
            )
    ...
    async def start_quiz(self, query, context):
        try:
                ...
                keyboard.append(
                    [
                        InlineKeyboardButton(
                            f"🔤 {option}", callback_data=f"answer_{option}"
                        )
                    ]
                )
            ...
            keyboard.append(
                [
                    InlineKeyboardButton(
                        "🔊 Hear Pronunciation", callback_data="hear_pronunciation"
                    )
                ]
            )
            reply_markup = InlineKeyboardMarkup(keyboard)
        ...
        try:
            ...
            with open(temp_path, "rb") as audio_file:
                await query.message.reply_voice(
                    voice=audio_file, caption=f"🔊 Pronunciation: '{english_word}'"
                )
    ...
    async def check_answer(self, query, context):
        try:
            ...
            keyboard = [
                [
                    InlineKeyboardButton(
                        "➡️ Next Question", callback_data="next_question"
                    )
                ],
                [
                    InlineKeyboardButton(
                        "🔊 Hear Correct Answer", callback_data="hear_pronunciation"
                    )
                ],
                [
                    InlineKeyboardButton(
                        "📊 View Progress", callback_data="view_progress"
                    )
                ],
    ...
    async def show_progress(self, query, context):
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    "📚 Continue Learning", callback_data="start_learning"
                )
            ],
            [InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")],
        ]
    ...
    async def show_main_menu(self, query, context):
        ...
        if not native_lang:
            await query.edit_message_text(
                "Please start with /start to select your language."
            )
            return
